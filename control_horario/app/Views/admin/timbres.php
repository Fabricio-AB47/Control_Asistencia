...
<main class="main-control">
  <div class="container">
      ...
      <div class="registro-campo">
        ...
        <select name="uid" id="uid-select" required>
          ...
          <?php foreach ($usuarios as $u): $sel = (!empty($uidSel) && (int)$uidSel===(int)$u['id_usuario']) ? 'selected' : ''; ?>
            <option value="<?= (int)$u['id_usuario'] ?>" data-rol="<?= htmlspecialchars($u['rol'] ?? '',ENT_QUOTES,'UTF-8') ?>" data-nombre="<?= htmlspecialchars($u['nombre'] ?? '',ENT_QUOTES,'UTF-8') ?>" <?= $sel ?>><?= htmlspecialchars($u['nombre'],ENT_QUOTES,'UTF-8') ?></option>
          <?php endforeach; ?>
        </select>
      </div>
      <div class="registro-campo">
    ...
    <form method="post" action="<?= $base ?>/index.php?r=admin&action=timbres_update" class="registro-formulario">
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  function actualizarRol(){
    const opt = sel.options[sel.selectedIndex] || sel.querySelector('option:checked');
    const rol = opt ? (opt.getAttribute('data-rol') || '') : '';
    rolInput.value = rol;
  }
  sel.addEventListener('change', actualizarRol);
  // Al cargar la página, poblar rol si ya hay usuario seleccionado
  actualizarRol();
});
</script>
